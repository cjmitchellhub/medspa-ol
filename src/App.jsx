...
// ─────────────────────────────────────────────────────────────────────────────
// DATA
// Homograph policy: words spelled same in EN/ES (e.g. insulin, lupus, cancer)
// belong at BEGINNER so learners get a confidence boost.
// Intermediate and advanced use genuinely new vocabulary.
// ─────────────────────────────────────────────────────────────────────────────
...
const BODY_SYSTEMS = [
  ...
  },
  {
    ...
    advanced: [
      ...
      { id:"nr15", en:"Guillain-Barré syndrome",es:"síndrome de Guillain-Barré",    phrase_en:"Your immune system is attacking your nerves.",       phrase_es:"Su sistema inmunológico está atacando sus nervios." },
...
function ReviewMode({ srs, onUpdateSRS, level }) {
  ...
  if (done) return (
    <div style={{ textAlign:"center", padding:"48px 20px" }}>
      ...
      <div style={{ color:"#94a3b8", marginBottom:28 }}>{correct >= queue.length * 0.75 ? "Excelente trabajo!" : "Keep practicing — you are improving!"}</div>
...
// ─────────────────────────────────────────────────────────────────────────────
// QUIZ — vocab and phrase pools stay separate
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div style={{ width:"min(600px,96vw)", display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <button onClick={() => {
        if (window.confirm("Reset all progress? This cannot be undone.")) {
          localStorage.removeItem(SRS_KEY); localStorage.removeItem(PROG_KEY); window.location.reload();
        }
      ...
      <div style={{ maxWidth:700, margin:"0 auto", padding:"20px 14px" }}>
        ...
        {mainTab === "learn" && (
          <>
            ...
            {terms.length === 0 ? (
              <div style={{ textAlign:"center", padding:"40px 20px", color:"#475569" }}>No cards available for this level yet.</div>
            ) : (
              <>
                {tab === "flashcards" && (
                  <Flashcard
